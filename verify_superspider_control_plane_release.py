...
def collect_report(root: Path) -> dict:
    ...
    script = superspider_root / "verify_control_plane_release.py"
    if not script.exists():
        return {
            "command": "verify-superspider-control-plane-release",
            "summary": "failed",
    ...
    if isinstance(payload, dict):
        ...
        payload["command"] = "verify-superspider-control-plane-release"
        payload["summary_text"] = payload.get("summary_text") or details or "superspider control-plane release gate"
        return payload
    return {
        "command": "verify-superspider-control-plane-release",
        "summary": "passed" if completed.returncode == 0 else "failed",
        "summary_text": details or "superspider control-plane release gate",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the superspider control-plane release gate from the spider repo")
    ...
    if args.json:
        ...
        print("verify-superspider-control-plane-release:", report["summary"])
