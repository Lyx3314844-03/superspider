...
使用方法:
    # 不使用代理
    python spider_jd_iphone17_proxy.py
    
    # 使用 HTTP 代理
    python spider_jd_iphone17_proxy.py --proxy http://127.0.0.1:7890
    
    # 使用 SOCKS5 代理
    python spider_jd_iphone17_proxy.py --proxy socks5://127.0.0.1:1080
    
    # 设置更多页数
    python spider_jd_iphone17_proxy.py --proxy http://127.0.0.1:7890 --pages 10
"""
...
class JDiPhone17Spider:
    ...
    def __init__(self, proxy_url: Optional[str] = None, max_pages: int = 5, delay: float = 2.0):
        # 结果存储
        ...
        # 输出目录
        self.output_dir = Path("output/jd_iphone17")
    ...
    async def search_and_parse(self, context: BrowserContext, keyword: str, page_num: int) -> List[Product]:
        ...
        try:
            ...
            # 等待商品列表加载
            try:
                await page.wait_for_selector('#J_goodsList .gl-item', timeout=15000)
            except:
                print("  等待商品列表超时，尝试继续解析...")
            ...
            if not goods_items:
                # 尝试备用选择器
                goods_items = await page.query_selector_all('.gl-item')
            
            if not goods_items:
                print(f"  未找到商品列表，可能页面结构变化或被反爬")
                # 保存页面截图用于调试
                await page.screenshot(path=f"debug_page_{keyword}_{page_num}.png")
                print(f"  已保存页面截图: debug_page_{keyword}_{page_num}.png")
                return products
    ...
    async def run(self):
        ...
        # 初始化浏览器
        context, browser, playwright = await self.init_browser()
        ...
        try:
            ...
            try:
                await page.goto('https://www.jd.com', wait_until='domcontentloaded', timeout=self.timeout)
                await page.wait_for_timeout(3000)
                print("已访问京东首页")
            except Exception as e:
                print(f"访问京东首页失败: {e}")
            ...
            for keyword in self.KEYWORDS:
                ...
                for page_num in range(1, self.max_pages + 1):
                    ...
                    # 随机延迟
                    delay = self.delay + random.uniform(1.0, 3.0)
    ...
    def save_results(self):
        ...
        # 保存商品
        json_filepath = self.output_dir / f"jd_iphone17_{timestamp}.json"
        json_data = [item.to_dict() for item in self.products]
        
        with open(json_filepath, "w", encoding="utf-8") as f:
            json.dump(json_data, f, ensure_ascii=False, indent=2)
        
        print(f"✓ 已保存 JSON: {json_filepath}")

        csv_filepath = self.output_dir / f"jd_iphone17_{timestamp}.csv"
    ...
    def print_stats(self):
        ...
        if self.products:
            ...
            print("\n未获取到商品数据。")
            print("\n提示：如果网络无法访问京东，请配置代理：")
            print("  python spider_jd_iphone17_proxy.py --proxy http://127.0.0.1:7890")
...
async def main():
    """主函数"""
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='京东 iPhone 17 价格爬虫')
    parser.add_argument('--proxy', type=str, help='代理服务器地址 (如: http://127.0.0.1:7890)')
    parser.add_argument('--pages', type=int, default=5, help='每个关键词爬取的页数 (默认: 5)')
    parser.add_argument('--delay', type=float, default=2.0, help='请求间隔秒数 (默认: 2.0)')
    
    args = parser.parse_args()
    
    print("\n" + "=" * 60)
    print("京东 iPhone 17 价格爬虫 v3.1.0")
