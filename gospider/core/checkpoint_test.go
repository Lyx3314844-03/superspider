...
func TestCheckpointManagerNew(t *testing.T) {
	...
	manager := NewCheckpointManager(tempDir, time.Second*5, 10)
	if manager == nil {
...
func TestCheckpointSaveAndLoad(t *testing.T) {
	tempDir := t.TempDir()
	manager := NewCheckpointManager(tempDir, 0, 10)
	defer manager.Close()
...
func TestCheckpointAutoSave(t *testing.T) {
	tempDir := t.TempDir()
	manager := NewCheckpointManager(tempDir, time.Second, 10)
	defer manager.Close()
...
// TestCheckpointSaveFromSpider - 测试从爬虫保存
