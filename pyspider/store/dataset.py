...
class Dataset:
    ...
    def _save_json(self, path: Path, indent: int = 2, ensure_ascii: bool = False) -> None:
        """保存为 JSON"""
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(self.items, f, indent=indent, ensure_ascii=ensure_ascii)
    
    def _save_csv(self, path: Path, delimiter: str = ',', encoding: str = 'utf-8') -> None:
        """保存为 CSV"""
        if not self.items:
            return
        
        with open(path, 'w', encoding=encoding, newline='') as f:
            writer = csv.DictWriter(f, fieldnames=self.items[0].keys(), delimiter=delimiter)
            writer.writeheader()
            writer.writerows(self.items)
    
    def _save_xml(self, path: Path, root_tag: str = 'dataset', item_tag: str = 'item') -> None:
        """保存为 XML"""
