...
class PersistentPriorityQueue:
    ...
    def _init_db(self):
        ...
        with self._lock:
            ...
            # 创建索引
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_priority ON requests(priority DESC)')
            ...
            # 初始化统计信息
            cursor.execute("INSERT OR IGNORE INTO stats (key, value) VALUES ('total_pushed', 0)")
            cursor.execute("INSERT OR IGNORE INTO stats (key, value) VALUES ('total_popped', 0)")
    ...
    def push(self, request: Request) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                # 更新统计
                cursor.execute("UPDATE stats SET value = value + 1 WHERE key = 'total_pushed'")
    ...
    def pop(self) -> Optional[Request]:
        ...
        with self._lock:
            ...
            # 删除已弹出的请求
            cursor.execute('DELETE FROM requests WHERE id = (SELECT id FROM requests ORDER BY priority DESC, id ASC LIMIT 1)')
            
            # 更新统计
            cursor.execute("UPDATE stats SET value = value + 1 WHERE key = 'total_popped'")
