...
class PersistentPriorityQueue:
    ...
    def _init_db(self):
        ...
        """)
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_priority ON {self.table_name}(priority DESC, created_at ASC)")
    ...
    def remove(self, url: str) -> bool:
        ...
        with self._lock:
            conn = self._get_connection()
            cursor = conn.execute(f"DELETE FROM {self.table_name} WHERE url = ?", (url,))
            conn.commit()
    ...
    def exists(self, url: str) -> bool:
        ...
        with self._lock:
            conn = self._get_connection()
            cursor = conn.execute(f"SELECT 1 FROM {self.table_name} WHERE url = ? LIMIT 1", (url,))
    ...
    def increment_retry(self, url: str) -> int:
        ...
        with self._lock:
            ...
            # 获取新的重试次数
            cursor = conn.execute(f"SELECT retry_count FROM {self.table_name} WHERE url = ?", (url,))
            row = cursor.fetchone()
    ...
    def get_stats(self) -> dict:
        ...
        with self._lock:
            ...
            # 按优先级分组
            cursor = conn.execute(f"SELECT priority, COUNT(*) FROM {self.table_name} GROUP BY priority")
            ...
            # 重试中的请求
            cursor = conn.execute(f"SELECT COUNT(*) FROM {self.table_name} WHERE retry_count > 0")
...
class RetryQueue:
    ...
    def __init__(self, max_retries: int = 3):
        ...
        self._queue = PersistentPriorityQueue(db_path="retry_queue.db", table_name="retries")
        self._lock = threading.RLock()
    ...
    def add_retry(self, item: QueueItem, reason: str = "") -> bool:
        ...
        # 延迟重试（指数退避）
        delay = 2 ** item.retry_count  # 1, 2, 4, 8...
