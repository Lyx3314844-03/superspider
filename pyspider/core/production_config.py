...
class ProductionConfig:
    ...
    def _load_from_env(cls) -> Dict[str, Any]:
        ...
        if os.getenv('CRAWLER_THREAD_COUNT'):
            env_config.setdefault('crawler', {})['thread_count'] = int(os.getenv('CRAWLER_THREAD_COUNT'))
        if os.getenv('CRAWLER_MAX_CONNECTIONS'):
            env_config.setdefault('crawler', {})['max_connections'] = int(os.getenv('CRAWLER_MAX_CONNECTIONS'))
        
        # 数据库配置
        if os.getenv('DATABASE_HOST'):
            env_config.setdefault('database', {})['host'] = os.getenv('DATABASE_HOST')
        if os.getenv('DATABASE_PASSWORD'):
            env_config.setdefault('database', {})['password'] = os.getenv('DATABASE_PASSWORD')
    ...
    def print_summary(self):
        ...
        logger.info(f"Message Queue: {self.message_queue.type} ({self.message_queue.host}:{self.message_queue.port})")
        logger.info("")
        logger.info(f"Monitor: {'enabled' if self.monitor.enabled else 'disabled'}")
        logger.info(f"Security: {'enabled' if self.security.enable_authentication else 'disabled'}")
        logger.info("=" * 60)
