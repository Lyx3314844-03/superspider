...
class SSRFProtection:
    ...
    def _is_private_ip(cls, ip_str: str) -> bool:
        ...
        try:
            ...
            return (
                ip.is_private  # 私有地址
                or ip.is_loopback  # 回环地址
                or ip.is_link_local  # 链路本地地址
                or ip.is_multicast  # 组播地址
                or ip.is_reserved  # 保留地址
                or ip.is_unspecified  # 未指定地址
            )
    ...
    def validate_redirect_chain(
        cls, initial_url: str, redirect_urls: List[str]
    ) -> bool:
        """
        验证重定向链的安全性
        ...
        for url in redirect_urls:
            ...
            if scheme not in cls.ALLOWED_SCHEMES:
                ...
                return False
            if initial_scheme == "https" and scheme != "https":
                logger.warning(f"检测到协议降级重定向：{initial_url} -> {url}")
                return False
        ...
        try:
            ...
            if parsed.hostname:
                try:
                    ...
                    info["ip_addresses"] = list(set(addr[4][0] for addr in addr_info))
                except Exception:
                    info["ip_addresses"] = []
