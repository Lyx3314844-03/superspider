...
class HLSParser:
    ...
    def parse_media_playlist(self, content: str, base_url: str = "") -> MediaPlaylist:
        """解析媒体播放列表"""
        playlist = MediaPlaylist(target_duration=0, media_sequence=0, segments=[])
...
class HLSDownloader:
    ...
    def download(self, m3u8_url: str, output_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(
                f"解析到 {len(playlist.segments)} 个分片，总时长 {playlist.total_duration:.1f}秒"
            )
    ...
    def _fetch_m3u8(self, url: str) -> Optional[str]:
        ...
        for attempt in range(self.retry_times):
            try:
                ...
                logger.warning(
                    f"获取 M3U8 失败 (尝试 {attempt+1}/{self.retry_times}): {e}"
                )
    ...
    def _download_best_stream(
        self, master_url: str, output_name: Optional[str] = None
    ) -> bool:
        """下载最佳流"""
        ...
        logger.info(
            f"选择最佳流：带宽 {best_stream.bandwidth}, 分辨率 {best_stream.resolution}"
        )
    ...
    def _download_segments(
        self, segments: List[MediaSegment], output_dir: Path
    ) -> bool:
        """下载所有分片"""
        self._downloaded_count = 0
        self._failed_count = 0
        ...
        logger.info(
            f"分片下载完成：成功 {self._downloaded_count}, 失败 {self._failed_count}, 成功率 {success_rate:.1%}"
        )

        return success_rate > 0.9

    def _download_segment(self, segment: MediaSegment, output_dir: Path) -> bool:
        """下载单个分片"""
    ...
    def _merge_segments(
        self, segments: List[MediaSegment], temp_dir: Path, output_file: Path
    ):
        """合并分片"""
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "success_rate": self._downloaded_count
            / max(self._downloaded_count + self._failed_count, 1),
        }
    ...
    def parse_mpd(self, content: str, base_url: str = "") -> Dict:
        ...
        try:
            ...
            for period in root.findall(".//mpd:Period", ns):
                period_info = {"duration": period.get("duration"), "adaptations": []}
                ...
                for adaptation in period.findall(".//mpd:AdaptationSet", ns):
                    ...
                    adaptation_template = adaptation.find("mpd:SegmentTemplate", ns)

                    adaptation_info = {"mimeType": mime_type, "representations": []}
                    ...
                    for rep in adaptation.findall(".//mpd:Representation", ns):
                        ...
                        if seg_template is None:
                            seg_template = adaptation_template
                        if seg_template is not None:
                            rep_info["initialization"] = seg_template.get(
                                "initialization"
                            )
                        ...
                        if seg_list is not None:
                            ...
                            if initialization is not None:
                                rep_info["initialization"] = initialization.get(
                                    "sourceURL"
                                )
...
class DASHDownloader:
    ...
    def __init__(
        self, output_dir: str = "downloads", max_workers: int = 10, timeout: int = 30
    ):
        self.output_dir = Path(output_dir)
        self.max_workers = max_workers
        self.timeout = timeout
    ...
    def _select_best_representation(
        self, mpd: Dict, mime_prefix: str
    ) -> Optional[Dict]:
        """选择最佳表示"""
        best_bandwidth = 0
        best_rep = None
    ...
    def _download_dash_segments(
        self, representation: Dict, base_url: str, output_dir: Path
    ) -> bool:
        """下载 DASH 分片"""
        ...
        if not segments:
            ...
            if seg_template:
                segments = self._build_segments_from_template(
                    representation, seg_template
                )
                if not segments:
    ...
    def _build_segments_from_template(
        self, representation: Dict, seg_template: Dict
    ) -> List[Dict]:
        ...
        period_duration = self._parse_iso8601_duration(
            representation.get("period_duration")
        )
    ...
    def _merge_dash_segments(
        self, representation: Dict, temp_dir: Path, output_file: Path
    ):
        """合并 DASH 初始化片段和媒体分片"""
        with open(output_file, "wb") as outfile:
    ...
    def _mux_dash_tracks(
        self, video_file: Path, audio_file: Path, output_file: Path
    ) -> bool:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        downloader = HLSDownloader(
            output_dir="downloads", max_workers=10, retry_times=3
        )
