...
class FFmpegExecutor:
    ...
    def _run_with_progress(
        ...
        total_duration = self.get_duration(cmd[cmd.index("-i") + 1]) if "-i" in cmd else 0
    ...
    def get_video_info(self, input_file: str) -> Optional[VideoInfo]:
        ...
        return VideoInfo(
            ...
            frame_rate=self._parse_frame_rate(video_stream.get('r_frame_rate', '0/1')) if video_stream else 0,
...
class FFmpegTools:
    ...
    def take_screenshots_batch(
        ...
        for i, timestamp in enumerate(range(0, int(duration), interval)):
            ...
            if self.take_screenshot(input_file, output_file, self._format_timestamp(timestamp), width):
    ...
    def add_watermark(
        ...
        args = [
            "-i", watermark_file,
            "-filter_complex", f"[0][1]overlay={pos}:format=auto:alpha={opacity}",
            "-c:a", "copy",
        ]
    ...
    def trim_video(
        ...
        args = [
            "-ss", start_time,
            "-t", duration if duration else "999999",
            "-c", "copy",
        ]
