...
class RedisDistributedQueue:
    ...
    def __init__(
        ...
        self.priority_queue_key = f"queue:{name}:priority"
    ...
    def push(self, task: CrawlTask) -> bool:
        ...
        with self._lock:
            ...
            # 添加到 URL 集合（用于去重）
            self.redis.sadd(f"{self.queue_key}:urls", self._url_key(task.url))
    ...
    def exists(self, url: str) -> bool:
        """检查 URL 是否在队列中"""
        return self.redis.sismember(f"{self.queue_key}:urls", self._url_key(url))
...
class RedisDistributedScheduler:
    ...
    def __init__(
        ...
        # 统计信息
        self.stats_key = f"stats:{spider_name}"
    ...
    def _update_stats(self, success: bool):
        ...
        if success:
            self.redis.hincrby(f"{self.stats_key}:success", timestamp, 1)
        else:
            self.redis.hincrby(f"{self.stats_key}:failed", timestamp, 1)
    ...
    def start(self, max_tasks: int = 0, timeout: int = 300):
        ...
        while self.running:
            ...
            if task:
                ...
                # 检查超时
                if timeout > 0 and (time.time() - self.stats['last_task_time']) > timeout:
                    logger.info("队列超时，退出")
                    break
