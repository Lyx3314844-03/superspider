...
class UserAgentRotator:
    ...
    def _initialize_pool(self):
        ...
        logger.info(f"UserAgentRotator 初始化完成，共 {len(self.ua_pool)} 个 User-Agent")
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'most_used_ua': max(self.usage_count, key=self.usage_count.get) if self.usage_count else 'N/A',
        }
...
class ProxyPool:
    ...
    def add_proxy(self, protocol: str, ip: str, port: int, 
    ...
    def add_proxies(self, proxies: List[ProxyInfo]):
        ...
        for proxy in proxies:
            self.add_proxy(
                ...
                proxy.username, proxy.password, proxy.country
            )
    ...
    def get_healthy_proxies(self) -> List[ProxyInfo]:
        ...
        return [
            ...
            if self.health_status.get(str(p), True)
        ]
    ...
    def _health_check_all(self):
        ...
        logger.info(f"代理健康检查完成：健康={healthy_count}, 不健康={unhealthy_count}, 总计={len(self.proxies)}")
    ...
    def _check_proxy_health(self, proxy: ProxyInfo) -> bool:
        ...
        try:
            ...
            response = requests.get(
                test_url,
                proxies=proxy.to_requests_proxy(),
