...
class DynamicWaitEnhanced:
    ...
    def wait_for_element_visible(
        self, selector: str, by=By.CSS_SELECTOR, timeout: int = None
    ) -> bool:
        """等待元素可见"""
        try:
    ...
    def wait_for_ajax(self, timeout: int = None) -> bool:
        ...
        try:
            ...
            wait.until(
                lambda d: (
                    d.execute_script("return jQuery.active == 0")
    ...
    def wait_for_condition(
        self, condition: Callable, timeout: int = None, message: str = ""
    ) -> bool:
        """等待自定义条件"""
        try:
            wait = WebDriverWait(
    ...
    def wait_for_elements_count(
        self, selector: str, count: int, by=By.CSS_SELECTOR, timeout: int = None
    ) -> bool:
        """等待元素数量"""
        try:
            wait = WebDriverWait(self.driver, timeout or self.timeout)
...
class ScrollLoaderEnhanced:
    ...
    def scroll_to_bottom(
        self, pause_ms: int = 1000, max_scrolls: int = 50, stable_threshold: int = 2
    ) -> int:
        """滚动到底部"""
        scroll_count = 0
...
class FormInteractorEnhanced:
    ...
    def select_option(
        self, selector: str, value: str = None, text: str = None, index: int = None
    ) -> bool:
        """选择选项"""
        try:
            element = self.driver.find_element(By.CSS_SELECTOR, selector)
    ...
    def drag_and_drop_by_offset(
        self, selector: str, x_offset: int, y_offset: int
    ) -> bool:
        """拖拽到偏移位置"""
        from selenium.webdriver.common.action_chains import ActionChains
