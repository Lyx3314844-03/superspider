...
def test_dependency_report_to_dict_normalizes_levels():
    report = SimpleNamespace(
        ...
        statuses=[
            ...
            SimpleNamespace(level="skip", name="Redis connection", message="not checked"),
        ],
    )
...
def test_check_browser_binary_accepts_absolute_existing_path(monkeypatch):
    monkeypatch.setattr(dependencies, "_browser_candidates", lambda: ("C:/Chrome/chrome.exe",))
    monkeypatch.setattr(dependencies.os.path, "isabs", lambda candidate: True)
    monkeypatch.setattr(dependencies.os.path, "exists", lambda candidate: candidate == "C:/Chrome/chrome.exe")
...
def test_run_dependency_doctor_builds_summary_and_exit_code(monkeypatch):
    monkeypatch.setattr(dependencies, "_check_python", lambda: dependencies.DependencyStatus("ok", "Python", "3.13.12"))
    monkeypatch.setattr(
        dependencies,
    ...
    monkeypatch.setattr(dependencies, "_check_output_dir", lambda output_dir: dependencies.DependencyStatus("ok", "Output", output_dir))
    monkeypatch.setattr(dependencies, "resolve_ffmpeg_path", lambda config_path=None: "C:/ffmpeg/ffmpeg.exe")
    monkeypatch.setattr(
        dependencies,
        "_check_ffmpeg",
        lambda ffmpeg_path: [dependencies.DependencyStatus("ok", "FFmpeg", ffmpeg_path)],
    )
    monkeypatch.setattr(dependencies, "_check_module", lambda name, module_name, optional: dependencies.DependencyStatus("ok", name, "installed"))
    monkeypatch.setattr(
        dependencies,
        "_check_redis_connection",
        lambda redis_url: dependencies.DependencyStatus("skip", "Redis connection", "not checked"),
    )
    monkeypatch.setattr(
        dependencies,
        "_check_browser_binary",
        lambda: dependencies.DependencyStatus("fail", "Browser binary", "missing browser"),
    )
