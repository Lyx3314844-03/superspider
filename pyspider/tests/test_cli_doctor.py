...
def test_doctor_command_reports_dependency_status(monkeypatch, capsys):
    fake_report = SimpleNamespace(
        ...
        statuses=[
            ...
            SimpleNamespace(level="ok", name="FFmpeg", message="available at C:/ffmpeg/bin/ffmpeg.exe"),
            SimpleNamespace(level="warn", name="Playwright", message="module not installed"),
        ],
        summary="2 passed, 1 warning, 0 failed, 0 skipped",
    )

    monkeypatch.setattr(video_downloader, "run_dependency_doctor", lambda args: fake_report, raising=False)
...
@pytest.mark.parametrize(
    ("argv", "patch_target"),
    [
        (["pyspider", "convert", "input.mp4", "output.mp4", "-f", "mp4"], "FFmpegTools"),
        (["pyspider", "info", "input.mp4"], "FFmpegExecutor"),
        (["pyspider", "screenshot", "input.mp4"], "FFmpegTools"),
    ],
)
def test_media_commands_surface_dependency_guidance(monkeypatch, capsys, argv, patch_target):
