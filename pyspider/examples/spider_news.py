...
class NewsSpider(Spider):
    ...
    def __init__(self):
        super().__init__(name="NewsSpider", thread_count=self.CONFIG["thread_count"])
    ...
    def parse(self, page):
        ...
        try:
            ...
            if self.is_list_page(page):
                # 提取文章链接
                article_links = page.response.css(
                    self.SELECTORS["article_link"]
                ).getall()
    ...
    def parse_article(self, page):
        ...
        try:
            ...
            publish_time = (
                page.response.css(self.SELECTORS["publish_time"])
                .get(default="")
                .strip()
            )
