...
class SocialMediaSpider(Spider):
    ...
    def __init__(self):
        super().__init__(
            name="SocialMediaSpider", thread_count=self.CONFIG["thread_count"]
        )
    ...
    def parse_user(self, page):
        ...
        try:
            # 提取用户数据
            username = (
                page.response.css(self.SELECTORS["username"]).get(default="").strip()
            )
            display_name = (
                page.response.css(self.SELECTORS["display_name"])
                .get(default="")
                .strip()
            )
            ...
            # 解析数字
            followers_text = (
                page.response.css(self.SELECTORS["followers"]).get(default="0").strip()
            )
            followers_count = self.parse_count(followers_text)

            following_text = (
                page.response.css(self.SELECTORS["following"]).get(default="0").strip()
            )
            following_count = self.parse_count(following_text)

            posts_text = (
                page.response.css(self.SELECTORS["posts"]).get(default="0").strip()
            )
            posts_count = self.parse_count(posts_text)

            # 提取头像
            avatar_url = (
                page.response.css(self.SELECTORS["avatar"]).get(default="").strip()
            )
            ...
            for link in post_links:
                if link and link.startswith("http"):
                    yield Request(
                        url=link, callback=self.parse_post, meta={"user_id": username}
                    )
    ...
    def parse_post(self, page):
        ...
        try:
            ...
            # 提取帖子数据
            content = (
                page.response.css(self.SELECTORS["post_content"])
                .get(default="")
                .strip()
            )
            ...
            # 提取时间
            created_at = (
                page.response.css(self.SELECTORS["post_time"]).get(default="").strip()
            )

            # 解析互动数据
            likes_text = (
                page.response.css(self.SELECTORS["post_likes"]).get(default="0").strip()
            )
            likes_count = self.parse_count(likes_text)

            comments_text = (
                page.response.css(self.SELECTORS["post_comments"])
                .get(default="0")
                .strip()
            )
            ...
            shares_text = (
                page.response.css(self.SELECTORS["post_shares"])
                .get(default="0")
                .strip()
            )
    ...
    def save_results(self):
        ...
        if self.OUTPUT_CONFIG["save_json"]:
            ...
            with open(users_file, "w", encoding="utf-8") as f:
                json.dump(
                    [u.to_dict() for u in self.users], f, ensure_ascii=False, indent=2
                )
            ...
            with open(posts_file, "w", encoding="utf-8") as f:
                json.dump(
                    [p.to_dict() for p in self.posts], f, ensure_ascii=False, indent=2
                )
...
def main():
    ...
    for username in spider.START_USERS:
        ...
        spider.add_request(
            Request(url=url, callback=spider.parse_user, meta={"depth": 0})
        )
