...
def crawl_youku_video(video_url, headless=True):
    ...
    try:
        with sync_playwright() as p:
            ...
            # 尝试等待视频播放器
            try:
                page.wait_for_selector('.video-player, #player, .yk-player', timeout=5000)
                print("   ✓ 检测到视频播放器")
            except:
                print("   ⚠️  未检测到视频播放器")
...
def extract_from_json(data, video_info):
    ...
    if isinstance(data, dict):
        ...
        if 'videoUrl' in data or 'download_url' in data:
            video_info['download_url'] = str(data.get('videoUrl', data.get('download_url', '')))
...
def parse_with_css(page, video_info):
    ...
    try:
        ...
        # 描述
        try:
            elem = page.query_selector('.video-info-detail, .summary, meta[name="description"]')
            if elem:
                if 'meta' in str(elem):
