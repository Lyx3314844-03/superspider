...
class YoukuVideoSpider(EnhancedSpider):
    ...
    def _navigate_to_video(self):
        ...
        urls_to_try = [
            ...
            (
                f"https://v.youku.com/v_show/{self.video_info.video_id}.html"
    ...
    def _wait_for_content(self):
        ...
        # 滚动页面触发加载
        AntiBot.human_like_scroll(self.browser, scroll_times=3, scroll_pause=1.0)
    ...
    def _extract_from_json(self, data: Dict):
        ...
        if isinstance(data, dict):
            ...
            if "views" in data or "viewCount" in data:
                self.video_info.views = str(
                    data.get("views", data.get("viewCount", ""))
                )

            # 缩略图
            if "thumbnail" in data or "poster" in data:
                self.video_info.thumbnail = data.get(
                    "thumbnail", data.get("poster", "")
                )

            # 视频 URL
            if "videoUrl" in data or "download_url" in data:
                self.video_info.download_url = data.get(
                    "videoUrl", data.get("download_url", "")
                )
    ...
    def _parse_with_regex(self, html: str):
        ...
        for pattern in title_patterns:
            ...
            if match:
                ...
                # 清理标题
                self.video_info.title = re.sub(
                    r"\s*-?\s*优酷\s*$", "", self.video_info.title
                )
                break
