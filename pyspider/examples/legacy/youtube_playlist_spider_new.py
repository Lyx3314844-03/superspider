    ...
    def _extract_from_json_contents(self, contents: dict, videos: list):
        ...
        def find_videos(obj, depth=0):
            ...
            if isinstance(obj, dict):
                # 检查是否是视频对象
                if (
                    "playlistPanelVideoRenderer" in obj
                    or "playlistPanelVideoLockedRenderer" in obj
                ):
                    renderer = obj.get("playlistPanelVideoRenderer") or obj.get(
                        "playlistPanelVideoLockedRenderer"
                    )
                    if renderer:
                        try:
                            video = VideoInfo()
                            video.title = renderer.get("title", {}).get(
                                "simpleText", ""
                            )

                            # 获取 URL
                            nav_endpoint = renderer.get("navigationEndpoint", {})
                            video.url = nav_endpoint.get("watchEndpoint", {}).get(
                                "url", ""
                            )
                            if not video.url and "videoId" in nav_endpoint.get(
                                "watchEndpoint", {}
                            ):
                            ...
                            if video.title and video.title not in [
