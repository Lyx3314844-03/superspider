...
class EcommerceSpider(Spider):
    ...
    def __init__(self):
        super().__init__(
            name="EcommerceSpider", thread_count=self.CONFIG["thread_count"]
        )
    ...
    def parse_product(self, page):
        ...
        try:
            ...
            # 提取价格
            price_text = (
                page.response.css(self.SELECTORS["price"]).get(default="0").strip()
            )
            price = self.parse_price(price_text)

            original_price_text = (
                page.response.css(self.SELECTORS["original_price"])
                .get(default="0")
                .strip()
            )
            ...
            # 提取描述
            description_parts = page.response.css(
            ...
            # 提取分类
            category = (
                page.response.css(self.SELECTORS["category"]).get(default="").strip()
            )

            # 提取评分
            rating_text = (
                page.response.css(self.SELECTORS["rating"]).get(default="0").strip()
            )
            rating = (
                float(rating_text.replace("星", "").replace("分", ""))
                if rating_text
                else 0.0
            )
            ...
            # 提取评论数
            review_count_text = (
                page.response.css(self.SELECTORS["review_count"])
                .get(default="0")
                .strip()
            )
            review_count = (
                int("".join(filter(str.isdigit, review_count_text)))
            ...
            # 提取库存状态
            stock_status = (
                page.response.css(self.SELECTORS["stock_status"])
                .get(default="")
                .strip()
            )
    ...
    def parse_reviews(self, page):
        ...
        for review_elem in reviews:
            try:
                user = (
                    review_elem.css(self.SELECTORS["review_user"])
                    .get(default="")
                    .strip()
                )
                rating_text = (
                    review_elem.css(self.SELECTORS["review_rating"])
                    .get(default="0")
                    .strip()
                )
                rating = (
                    int(rating_text.replace("星", "").replace("分", ""))
                    if rating_text
                    else 0
                )

                title = (
                    review_elem.css(self.SELECTORS["review_title"])
                    .get(default="")
                    .strip()
                )
                content = (
                    review_elem.css(self.SELECTORS["review_content"])
                    .get(default="")
                    .strip()
                )
                date = (
                    review_elem.css(self.SELECTORS["review_date"])
                    .get(default="")
                    .strip()
                )

                helpful_text = (
                    review_elem.css(self.SELECTORS["review_helpful"])
                    .get(default="0")
                    .strip()
                )
                helpful_count = (
                    int("".join(filter(str.isdigit, helpful_text)))
                    if helpful_text
                    else 0
                )
...
def main():
    ...
    for url in spider.START_URLS:
        spider.add_request(Request(url=url, callback=spider.parse, meta={"depth": 0}))
