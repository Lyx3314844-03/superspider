...
class MetricsCollector:
    ...
    def get_metrics(self) -> PerformanceMetrics:
        ...
        with self._lock:
            # 响应时间统计
            recent_times = [rt for t, rt in self.response_times if t > window_start]
...
class SpiderMonitor:
    ...
    def record_page_crawled(
        self, url: str, status_code: int, bytes_downloaded: int = 0
    ):
        """记录页面爬取"""
        with self._lock:
...
class MonitorCenter:
    ...
    def get_all_stats(self) -> dict:
        ...
        with self._lock:
            return {
                name: monitor.get_stats() for name, monitor in self.monitors.items()
            }
