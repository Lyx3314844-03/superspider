...
class CircuitBreaker:
    ...
    def allow(self) -> bool:
        ...
        with self._lock:
            if self.state == "open":
                if self.last_failure and (datetime.now() - self.last_failure).total_seconds() > self.timeout:
...
class AdaptiveRateLimiter:
    ...
    def __init__(self, initial_delay: float = 1.0, min_delay: float = 0.1,
                 max_delay: float = 60.0, target_response_time: float = 2.0):
