...
def collect_report(root: Path) -> dict:
    ...
    script = superspider_root / "verify_control_plane_package.py"
    if not script.exists():
        return {
            "command": "verify-superspider-control-plane-package",
            "summary": "failed",
    ...
    if isinstance(payload, dict):
        ...
        payload["command"] = "verify-superspider-control-plane-package"
        return payload
    details = "\n".join(part for part in (completed.stdout.strip(), completed.stderr.strip()) if part).strip()
    return {
        "command": "verify-superspider-control-plane-package",
        "summary": "passed" if completed.returncode == 0 else "failed",
        "summary_text": details or "superspider control-plane package gate",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the superspider control-plane package gate from the spider repo")
    ...
    if args.json:
        ...
        print("verify-superspider-control-plane-package:", report["summary"])
