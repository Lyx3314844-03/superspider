 ...
 */
public class JDiPhone17Spider implements PageProcessor {
    
    // 搜索关键词列表
    private static final String[] KEYWORDS = {"iPhone 17", "苹果17", "Apple iPhone 17"};
    ...
    // 配置
    private int maxPages = 5;           // 每个关键词爬取的最大页数
    ...
    // 输出目录
    private static final String OUTPUT_DIR = "output/jd_iphone17";
    ...
    public JDiPhone17Spider() {
        ...
        if (proxyServer != null && !proxyServer.isEmpty()) {
            System.out.println("使用代理: " + proxyServer);
            // HttpClientDownloader 支持代理配置
        }
    }
    ...
    public void process(Page page) {
        ...
        try {
            ...
            for (int i = 0; i < productElements.size(); i++) {
                ...
                if (!nameLower.contains("iphone 17") && 
                    !nameLower.contains("iphone17") && 
                    !nameLower.contains("苹果17") &&
                    !nameLower.contains("apple 17")) {
                    continue;
                }
    ...
    private void saveResults() throws Exception {
        ...
        Path jsonPath = outputDir.resolve("jd_iphone17_" + timestamp + ".json");
        ...
        // 保存 CSV
        Path csvPath = outputDir.resolve("jd_iphone17_" + timestamp + ".csv");
    ...
    private void printStats() {
        ...
        if (!products.isEmpty()) {
            ...
            int thirdCount = 0;
            for (Map<String, Object> p : products) {
                String shopName = (String) p.get("shop_name");
                if (shopName != null && !shopName.isEmpty()) {
                    shops.add(shopName);
                }
                if ("自营".equals(p.get("shop_type"))) {
                    selfCount++;
                } else {
                    thirdCount++;
                }
            ...
            System.out.println("\n店铺统计:");
            System.out.println("  店铺总数: " + shops.size());
            System.out.println("  自营商品: " + selfCount);
            System.out.println("  第三方商品: " + thirdCount);
            ...
            for (int i = 0; i < Math.min(10, sorted.size()); i++) {
                ...
                double price = (double) p.getOrDefault("price", 0.0);
                String priceStr = price > 0 ? String.format("¥%.2f", price) : "价格未知";
                String name = (String) p.get("name");
                String shopType = (String) p.get("shop_type");
                System.out.println("  " + (i + 1) + ". " + priceStr + " - " + 
                    (name.length() > 50 ? name.substring(0, 50) : name) + " (" + shopType + ")");
            }
        } else {
            System.out.println("\n未获取到商品数据。");
            System.out.println("\n提示：如果网络无法访问京东，请：");
            System.out.println("  1. 配置代理服务器");
            System.out.println("  2. 确保网络可以访问京东");
        }
        ...
        for (String keyword : KEYWORDS) {
            ...
            for (int page = 1; page <= maxPages; page++) {
                ...
                try {
                    ...
                    totalPages++;
                    System.out.println("  第 " + page + " 页: 累计 " + products.size() + " 个商品");
    ...
    public static void main(String[] args) {
        System.out.println("\n============================================================");
        System.out.println("京东 iPhone 17 价格爬虫 (JavaSpider)");
        System.out.println("目标：爬取京东所有苹果17的价格信息");
        System.out.println("============================================================\n");
        ...
        for (int i = 0; i < args.length; i++) {
            if ("--pages".equals(args[i]) && i + 1 < args.length) {
                maxPages = Integer.parseInt(args[i + 1]);
            } else if ("--proxy".equals(args[i]) && i + 1 < args.length) {
                proxy = args[i + 1];
            }
        }
        
        JDiPhone17Spider spider = new JDiPhone17Spider();
        spider.maxPages = maxPages;
        spider.proxyServer = proxy;
